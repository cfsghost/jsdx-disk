		...
		function(err, iface) {
			...
			iface.on('InterfacesAdded', function(objectPath, data) {
				...
				if (objectPath.search('/org/freedesktop/UDisks2/drives') == 0) {
					self.emit('DriveAdded', objectPath, data);
					return;
				}

				// Block Device
				if (objectPath.search('/org/freedesktop/UDisks2/block_devices') == 0) {
					self.emit('BlockDeviceAdded', objectPath, data);
					return;
				}
			});
			...
			iface.on('InterfacesRemoved', function(objectPath, data) {
				...
				// Drive
				if (objectPath.search('/org/freedesktop/UDisks2/drives') == 0) {
					self.emit('DriveRemoved', objectPath, data);
					return;
				}

				// Block Device
				if (objectPath.search('/org/freedesktop/UDisks2/block_devices') == 0) {
					self.emit('BlockDeviceRemoved', objectPath, data);
					return;
				}
			});
